import { d as escape_html, c as attr, j as stringify, k as ensure_array_like } from './index2-CFsRFmOn.js';
...
function _page($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
      ...
      $$renderer2.push(`<div class="h-96 bg-sky-100/75 hover:bg-sky-500"><div class="m-6"><p class="mb-2 h-32 text-[32pt] font-bold tracking-tight text-gray-900 dark:text-white">${escape_html(block1_item.title)}</p> <p class="mb-3 leading-tight font-normal text-gray-700 dark:text-gray-400">${escape_html(block1_item.description)}</p> <button class="inline-flex w-40 cursor-pointer items-center justify-center rounded-lg bg-primary-50 px-5 py-2.5 text-center text-sm font-medium text-white focus-within:ring-4 focus-within:ring-primary-300 focus-within:outline-hidden hover:bg-primary-800">Lebih jauh `);
      RightArrow($$renderer2);
      $$renderer2.push(`<!----></button></div></div>`);
    }
    $$renderer2.push(`<!--]--></div> <div class="mt-10 grid h-screen grid-cols-3 grid-rows-3 content-center gap-2 text-[20pt]"><a href="/" class="content-center rounded-[20px] bg-primary-50 text-center text-white"><p>${escape_html(data.data.block_3s[0].title)}</p></a> <a href="/" class="col-start-2 row-start-2 content-center rounded-[20px] bg-primary-50 text-center text-white"><p>${escape_html(data.data.block_3s[1].title)}</p></a> <a href="/" class="col-start-3 row-start-3 content-center rounded-[20px] bg-primary-50 text-center text-white"><p>${escape_html(data.data.block_3s[2].title)}</p></a></div> <div class="mx-auto mt-10 flex h-screen flex-col justify-center"><div class="text-center"><p>${escape_html(data.data.lebih_lanjut)}</p> <button class="mt-4 cursor-pointer rounded-[30px] bg-primary-50 px-8 py-4 text-white">Selengkapnya..</button></div></div>`);
  });
}

export { _page as default };
//# sourceMappingURL=_page.svelte-9gy1xHhE.js.map
